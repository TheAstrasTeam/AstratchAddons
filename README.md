...
The plugin center of Astratch — source code, compiled release artifacts, and build scripts.
...
release branch (append-only, never removes old versions):
  <id>@v<version>/                        ← flat release dirs (addon.js, info.json, assets/, i18n/)
  registry.json                           ← store entry (generated during release, not on main)
   │
   ▼
Astratch (runtime)
...
scripts/
  ...
  prepare-release.mjs         Generates registry.json + copies releases to flat <id>@v<version>/ dirs
...
<id>@v<version>/              ← flat release dirs
  ...
  assets/
  i18n/
registry.json                 ← generated store entry (not on main)
```
...
Generated by `prepare-release.mjs` and deployed **only to the release branch**. Not committed to main.
  ...
  "addons": [
    {
      ...
      "description": "...",
      "license": "MIT",
      "icon": "example@v1.0.0/assets/icon.svg",
      "defaultEnabled": false,
      "settings": [...],
      "i18n": { "en": "example@v1.0.0/i18n/en.json", "zh-CN": "example@v1.0.0/i18n/zh-CN.json" },
      "astratch": { "minVersion": "0.1.0" },
      "versions": ["1.0.0", "1.1.0"],
      "download": "example@v1.0.0/"
    }
  ]
}
```

Icon and i18n use **file paths** (relative to release root), not inline data. The client fetches them on demand.

## Releasing

1. Bump `version` in `addons/<id>/info.yaml`
2. Commit to `main`
3. CI builds `releases/<new-version>/`, generates `registry.json`, and force-pushes the `release` branch
...
npm run prepare-release  # generate registry.json + copy releases to flat <id>@v<version>/ dirs
```

GitHub Actions runs `npm run check` on every push to `main`, then runs `prepare-release` and force-pushes the `release` branch with the generated `registry.json` and compiled artifacts. To release an addon, add its id to `addons.json` and commit the source.
